...
Delivery is **pull-based**: a self-hosted runner on the deploy host authenticates to GHCR with the job's own token, refreshes the compose file from `main`, runs `docker compose pull && docker compose --profile prod up -d` pinned to the SHA just built, and waits on the container healthcheck — failing the run if the app never comes up. Nothing connects *into* the server, which is what makes it work against a host that is IPv6-native with a dynamic IPv4 while GitHub's own runners are IPv4-only ([ADR 0030](./docs/decisions/0030-self-hosted-runner-rollout.md)). The deploy path stores no repository secrets at all.
