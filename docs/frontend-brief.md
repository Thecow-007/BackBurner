...
- **Filter bar** (maps 1:1 to `GET /tasks` query params):
  ...
  - "Clear filters" affordance, visible only when any filter is active.
...
- **Load more** button when `next_cursor` is non-null (cursor pagination, default page 50).
...
**Elements.**
- Lane select — `scrape` / `report`. Required. Remembers the last-used lane within the session.
- **Duration** (`duration_ms`) — numeric field, milliseconds, with a live humanized preview ("10 000 ms ≈ 10 s"). Optional: left blank, the engine picks a random 3–15 s duration. Validation: positive integer ≤ 600 000.
...
- **Outcome** select — three honest options: **Succeed** (default); **Fail — retryable** (`fail: true`), helper text "Worker returns a retryable error; the engine auto-retries with backoff until the attempt budget runs out"; **Fail — permanent** (`fail_permanent: true`), helper text "Worker returns a non-retryable error; the task lands in failed immediately, no auto-retries". The mock worker makes every failure scenario reproducible; this control is how demos and tests exercise both of the spec's permanent-failure flavors.
- **Advanced** (collapsed by default): Max attempts (`max_attempts`) — integer 1–10; helper text: "Automatic retries with backoff before the task lands in failed."
- **Submit** button.
...
**Behavior.** Submit issues `POST /tasks` with `{lane, params: {duration_ms?, fail?, fail_permanent?}, max_attempts?}`. The response (`201`, full task object with `id` and `handle`) identifies the task, but the confirmation panel appears only when the `accepted` event lands in the store (§6.4). The handle comes back well under a second, so this feels instant; the discipline matters because the store is the single source of truth.

**States.**

| State | Presentation |
|---|---|
| Idle | Form with prefilled lane/duration defaults. |
| Invalid | Inline per-field errors on blur and on submit attempt (duration range, max-attempts range). Submit disabled while invalid. |
...
4. **Zero polling.** No `setInterval` refreshes, no refetch-on-focus, no refresh buttons. Reads happen at exactly four moments: hydration snapshot, filter/pagination changes (user-driven), detail-screen mount (navigation-driven), and the single event-driven backfill fetch (§5.1). Everything else is push.
...
- Selecting a lane prefills the duration field from its stored default; no stored default leaves the field blank (server default: random 3–15 s).
...
- Helper text under the field always states the current situation: "Default for scrape: 10 s" / "No default set — the engine picks 3–15 s".
...
- **Failed toast:** "**scrape-1** failed" plus `reason`, with a retryable indicator. **Persists until dismissed** — failures must not evaporate unseen.
...
| `lane` | string | `scrape` or `report` in this deployment. |
| `params` | object | Submit-time parameters (`duration_ms`, `fail`, `fail_permanent`, plus any pass-through keys). |
...
```json
{ "tasks": [ /* task objects */ ], "as_of": 1234, "next_cursor": "opaque-or-null" }
```

Query params: `status`, `lane`, `from`, `to` (ISO dates, on `created_at`), `sort` (`created_at|updated_at` + `:asc|:desc`, default `created_at:desc`), `limit` (default 50, max 200), `cursor`. `as_of` is the SSE resume point (§5).
...
| Desktop | > 1024 px | Task list as a table: columns for handle, lane, status, attempts, created, updated, actions; sortable headers mirror the sort control. Filters inline in one row. Detail two-column: identity/params/actions left, timeline and error/result right. Notification center as an anchored popover. Content max-width ~1200 px, centered. |
