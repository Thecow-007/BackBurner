...
npm run seed -- --tasks 300        # seed data; prints raw keys for daniel/reviewer/newcomer once
                                   # (newcomer has zero tasks — the empty-state demo account);
                                   # --reset removes seeded rows only
...
- **Five lanes, all mock-worker-backed, in registration order:** `scrape`, `report`, `convert`, `build`, `test`. The order is contract — it is what `counts.lanes` reports and therefore what the sidebar and submit picker render. `build` is the long-running lane.
- **The mock worker is driven by params.** `params.duration_ms` (1–600000) sets the sleep; omitted, a random value from **the lane's own range** is chosen at submit and written back into stored params so retries are deterministic — 3000–15000 ms everywhere, 20000–90000 ms on `build`. `params.fail: true` returns a retryable failure; `params.fail_permanent: true` a non-retryable one (straight to `failed`, budget ignored; operator retry still allowed); `params.fail_times: n` (1–9) returns a retryable failure while `ctx.attempt <= n` and succeeds after that — the flaky-then-recovers path. Precedence: `fail_permanent` > `fail` > `fail_times`. **With no outcome param a job always succeeds** — the default outcome is deterministic, never a server-side dice roll; the SPA rolls its own dice and submits explicit params. Extra params keys pass through untouched. Per-lane ranges reach clients as `counts.lane_defaults`, added by the API (the engine is lane-agnostic).
- **`WorkerContext` is `{ signal, attempt, maxAttempts }`.** `attempt`/`maxAttempts` are the values the claim computed and journaled onto that claim's `running` transition — never recompute or infer them elsewhere, or the worker and the history endpoint will disagree. A spec-shaped one-argument worker stays assignable.
- **`?uncollected=true` and `?q=` are the two extension list filters.** `uncollected` accepts only the literal string `true` (anything else is 400) and reuses the *exact* SQL predicate behind `counts.uncollected`. `q` (1–64 chars) matches handle/id by equality or prefix, is rank-ordered and unpaginated (`next_cursor` always `null`), and is a 400 when combined with `sort` or `cursor`. Both are parsed once in the engine's `filters.ts` so `list()` and `counts()` can never read them differently.
